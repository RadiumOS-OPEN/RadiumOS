...
impl Sha512 {
    ...
    pub(super) fn update(&mut self, mut input: &[u8]) {
        ...
        if self.buffer_len > 0 {
            let count = input.len().min(128 - self.buffer_len);
    ...
    pub(super) fn finish(mut self) -> [u8; 64] {
        let bit_len = (self.message_len as u128).wrapping_mul(8).to_be_bytes();
        ...
        self.buffer[112..].copy_from_slice(&bit_len);
        let block = self.buffer;
        Self::compress(&mut self.state, &block);
